...
public final class Card implements Comparable {
	...
	public Card(int suit, int value, boolean visible) {
		...
		if ((suit > SPADES) | (suit < UNIQUE_SUIT)) {
			this.suit = UNKNOWN;
		} else {
			this.suit = suit;
		}
	 ...
	 * For poker we sort first by value and second by suit.
	 */
	public int compareTo(Object o) {
		Card argCard = (Card) o;

		// Compare values first
		int vdiff = (this.value - argCard.cardValue());
		if (vdiff != 0) {
			return (vdiff);
		}

		// if value is same, compare suits
		return (this.suit - argCard.cardSuit());
	}

	private static String [] suitStrings = {"Clubs", "Diamonds", "Hearts", "Spades"};
	...
	/**
	 * Determine if the card has the same value as the given card.
	 *
	 * @param   otherCard   The other card to compare with
	 *
	 * @return if this card has the same value as the other card.
	 */
