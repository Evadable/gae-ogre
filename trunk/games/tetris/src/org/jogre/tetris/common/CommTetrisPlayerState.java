 ...
 */
package org.jogre.tetris.common;
 ...
 * @version Alpha 0.2.3
 */
public class CommTetrisPlayerState extends CommTableMessage {

    // Constants
    private static final int NUM_OF_CELLS = TetrisPlayerModel.NUM_OF_COLS * TetrisPlayerModel.NUM_OF_ROWS;    
    private static final int RLE_START = 8;
    ...
    // XML variables
    public static final String XML_NAME = "tetris_player_state";
    public static final String XML_ATT_SEAT = "seat";
    
    /**
     * Constructor for a tetris player state which compressed the grid data using 
     * RLE.  
     * 
     * @param gridData   Uncompressed grid data.
     */
    public CommTetrisPlayerState (int seatNum, int [][] gridData) {
        this.seatNum  = seatNum;
        this.gridData = gridData;
        
        compress ();			// TODO - EXTERNALISE TO A DEDICATED COMPRESSION UTILITY CLASS?
    }
     ...
     * @param message
     */
    public CommTetrisPlayerState (XMLElement message) {
    ...
    private void decompress () {
        this.gridData = new int [TetrisPlayerModel.NUM_OF_COLS][TetrisPlayerModel.NUM_OF_ROWS];
        ...
        for (int i = 0; i < length; i++) {
            ...
            // If the value is less than number of different tetris pieces then its uncompressed
            if (cv < RLE_START) { 
                setGridData (index++, cv);
            }
            // Otherwised its compressed using RLE
            else {
    ...
    private int getGridData (int index) {
        return this.gridData [index % TetrisPlayerModel.NUM_OF_COLS]
                             [index / TetrisPlayerModel.NUM_OF_COLS];
    }
    ...
    private void setGridData (int index, int value) {
        this.gridData [index % TetrisPlayerModel.NUM_OF_COLS]
                      [index / TetrisPlayerModel.NUM_OF_COLS] = value;
    }
